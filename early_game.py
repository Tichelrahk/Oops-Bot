...
class EarlyGame(Strategy):
        def __init__(self):
            ...
            self._is_build_order_complete = False
            self._build_order_check = 0

        async def build_order(self, ccs, oops_bot):
            
            

            if not self._is_build_order_complete and (oops_bot.already_pending(UnitTypeId.FACTORY) + oops_bot.structures(UnitTypeId.FACTORY).ready.amount) > 0:
                    self._is_build_order_complete = True
                    await oops_bot.chat_send(f"Build order finished")
                        
            if not self._is_build_order_complete:
                #standard build order
                if self._build_order_check == 0 and oops_bot.supply_workers >= 13 and oops_bot.can_afford(UnitTypeId.SUPPLYDEPOT) and oops_bot.already_pending(UnitTypeId.SUPPLYDEPOT) < 1:
                    await oops_bot._base_command.build_supply(oops_bot)

                    self._build_order_check = 1
                    await oops_bot.chat_send(f"(building supply depot)")

                if self._build_order_check == 1 and oops_bot.structures(UnitTypeId.SUPPLYDEPOT).ready and oops_bot.can_afford(UnitTypeId.BARRACKS):
                    await oops_bot._base_command.builder(oops_bot, UnitTypeId.BARRACKS, 12)
                    self._build_order_check = 2
                    await oops_bot.chat_send(f"(building rax)")


                if self._build_order_check == 2 and oops_bot.can_afford(UnitTypeId.COMMANDCENTER) and (oops_bot.townhalls.amount) < 2:
                    await oops_bot.expand_now()
                    self._build_order_check = 3
                ...
                for b in oops_bot.structures(UnitTypeId.BARRACKS).ready.idle:
                    if self._build_order_check >= 2 and oops_bot.can_afford(UnitTypeId.BARRACKSTECHLAB) and not b.has_add_on and oops_bot.structures(UnitTypeId.BARRACKSTECHLAB).amount < 1:
                ...
                if self._build_order_check >= 3 and oops_bot.can_afford(UnitTypeId.FACTORY) and  (oops_bot.already_pending(UnitTypeId.FACTORY) + oops_bot.structures(UnitTypeId.FACTORY).ready.amount) < 2:
                    await oops_bot._base_command.builder(oops_bot, UnitTypeId.FACTORY, 10)
        ...
        async def industrialize(self, ccs, oops_bot):
            ...
            if self._is_build_order_complete:
